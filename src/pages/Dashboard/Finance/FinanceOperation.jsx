...
import {
  ...
  Calculator, ArrowRight, Eye
} from 'lucide-react';

function FinanceOperation() {
  const navigate = useNavigate();

  const reportCategories = [
    {
      title: "Payment",
      description: "Payment recording and management",
...
>              Finance Operations
          ...
          <p className="text-slate-600 text-sm md:text-base font-medium text-center max-w-2xl">
            Complete financial management for payments and expenses
          </p>
        </motion.div>
                    ...
                    >
                      <div className="bg-gradient-to-br from-white/90 to-white/70 backdrop-blur-sm border border-white/50 rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
                        ...
                        <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-200/50">
                          <button className="flex items-center gap-2 text-slate-500 hover:text-orange-600 transition-colors duration-300">
     ...
     <div>
      ...
      <div className="fixed top-20 left-10 w-20 h-20 bg-orange-500/20 rounded-full opacity-60 animate-bounce" style={{ animationDelay: "0s" }}></div>
      <div className="fixed top-40 right-10 w-16 h-16 bg-red-500/20 rounded-full opacity-60 animate-bounce" style={{ animationDelay: "1s" }}></div>
      <div className="fixed bottom-20 left-20 w-12 h-12 bg-pink-500/20 rounded-full opacity-60 animate-bounce" style={{ animationDelay: "2s" }}></div>
    </div>
  );
}
